              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                    ...
                    {session.rawLogs && (session.rawLogs.vulnerable.length + session.rawLogs.secure.length) > 0 && (
                      <>
                        <span>·</span>
                        <span className="rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 font-mono text-[10px] text-blue-600">
                          Raw HTTP {session.rawLogs.vulnerable.length + session.rawLogs.secure.length}건
                        </span>
                      </>
                    )}
                  </div>
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              >
                <div className="flex items-center gap-2">
                  <span>서버 수신 Raw HTTP 로그</span>
                  {!session.rawLogs ? (
                    <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[9px] text-slate-400">
                      데이터 없음
                    </span>
                  ) : (
                    <span className="rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 text-[9px] font-medium text-blue-600">
                      취약 {session.rawLogs.vulnerable.length}건 / 보안 {session.rawLogs.secure.length}건
                    </span>
                  )}
                </div>
                ...
                <div className="border-t border-slate-200 p-4">
                  {!session.rawLogs ? (
                    <p className="py-4 text-center text-xs text-slate-400">
                      이 세션은 Raw HTTP 로그 수집 전 저장된 세션입니다. 새 공격을 실행하면 로그 데이터가 포함됩니다.
                    </p>
                  ) : (
                    <div className="grid gap-4 lg:grid-cols-2">
                      <div>
                        <div className="mb-2 text-xs font-semibold text-red-600">
                          취약 환경 ({session.rawLogs.vulnerable.length}건)
                        </div>
                        {session.rawLogs.vulnerable.length === 0 ? (
                          <p className="py-4 text-center text-xs text-slate-400">수신된 요청 없음</p>
                        ) : (
                      ...
                      <div>
                        <div className="mb-2 text-xs font-semibold text-emerald-600">
                          보안 환경 ({session.rawLogs.secure.length}건)
                        </div>
                        {session.rawLogs.secure.length === 0 ? (
                          <p className="py-4 text-center text-xs text-slate-400">수신된 요청 없음 (WAF/SG가 서버 도달 전 차단)</p>
                        ) : (
                          <div className="flex max-h-96 flex-col gap-2 overflow-y-auto">
